...
- `start`: the first token position that still needs to be computed. If there is no prefix cache, `start = 0`. If the first four tokens are already cached, `start = 4`.
