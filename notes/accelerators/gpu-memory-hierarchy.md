...
**Registers** are the closest (=fastest) memory resource to execution. It also has the smallest capacity. Usually the faster bandwidth it has, the smaller capacity it has. This is not something fundamental, but only using SRAMs in a chip is not that cost-efficient and very expensive. Therefore computer architecture, including GPUs, usually use memory hierarchy and cache to have similar speed as SRAM but large storage like DRAM (much more cost-efficient; hardware-software hack basically).

In CUDA C/C++, ordinary automatic variables often live in registers when the compiler can keep them there.<sup><a href="#reference-1">[1]</a></sup> 

If a kernel needs too many registers per thread, the compiler may spill values into local memory, which is much slower to fetch. High register use can also reduce occupancy because each resident thread block consumes part of the SM's register file.
...
Shared memory is fast(about ~100x faster than global memory) on-chip memory that all threads in the same thread block can access.<sup><a href="#reference-1">[1]</a></sup> It is one of the most important memory for CUDA performance because it lets a block explicitly stage and reuse data.

On many NVIDIA architectures, shared memory and L1 cache use the same physical on-chip resource, often called the _unified data cache_. The portion reserved for shared memory can be configured per kernel on supported devices.<sup><a href="#reference-3">[3]</a></sup>

Common examples include tiled matrix multiplication, stencil kernels, reductions, and data-layout transformations before Tensor Core operations.
...
Local memory is off-chip memory, which means it resides in DRAM. Usually large logical data structures or local arrays are saved here.

From a performance point of view, local memory should usually be treated like global-memory traffic with thread-private semantics.
...
Global memory is the main device memory space, often interchangeably called as GPU VRAM or HBM.

Global memory is largest but the slowest. CUDA performance often depends on making global-memory accesses coalesced, reducing unnecessary transfers, and reusing loaded data in registers or shared memory.
...
Constant memory is a read-only device memory space. It is useful when many threads read the same values, especially when threads in a warp read the same address and the hardware can broadcast the value efficiently.<sup><a href="#reference-1">[1]</a></sup>

In short, constant memory is a specialized path for small read-only data with favorable access patterns.
